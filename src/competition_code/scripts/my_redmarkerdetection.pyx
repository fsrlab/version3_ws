...
def sort_number_label(np.ndarray[DTYPE_t, ndim=3] frame,np.ndarray[DTYPE_t, ndim=1] seg_papram):
    ...
    for i in quads_prj:
        ...
        center_list_xy.append((center_x,center_y))
    sorted_xy = sorted(center_list_xy,key=lambda x:x[1],reverse = False)
    sorted_y3 = sorted_xy[0:3]
    sorted_xy3 = sorted(sorted_y3,key=lambda x:x[0],reverse = False)
    ...
    for i in quads_prj:
        ...
        center_x = int(M['m10']/M['m00'])
        center_y = int(M['m01']/M['m00'])
        compare_xy=(center_x,center_y)
        for k in range(len(sorted_xy3)):
            if compare_xy == sorted_xy3[k]:
                points_src = np.array([[(i[0,0,0],i[0,0,1])],
                                [(i[1,0,0],i[1,0,1])],
                                [(i[2,0,0],i[2,0,1])],
                                [(i[3,0,0],i[3,0,1])]],dtype = "float32")
                ...
                match_threshold = 350000
                if min_diff < match_threshold:
                    if min_diff_target not in sorted_ID:
                        sorted_ID[k] = min_diff_target
    if len(sorted_ID)==3:                  
        return sorted_ID
